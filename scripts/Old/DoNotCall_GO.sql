	...
	-- atualizando campo: Telefone 
	update ATU_PROCON set telefone = convert ( nvarchar, convert( bigint ,telefone ) );
	...
	update ATU_PROCON set obs = 'fone invalido' where obs is null and substring(fone,1,1) in ('0','1');
	update ATU_PROCON set obs = 'fone invalido' where obs is null and substring(fone,1,1) in ('0','1');
	...
	update ATU_PROCON set obs = 'ddd invalido' where obs is null and  ddd not in ( select ddd from UFBR where uf = 'GO' group by ddd )
	...
	insert into DoNotCall_GO ( ddd ,telefone ,telbase ,data ) 
