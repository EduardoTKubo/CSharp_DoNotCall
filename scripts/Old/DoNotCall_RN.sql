	...
	use procon
		...
		select ddd from UFBR where uf = 'RN' group by ddd -- 84
		select * from ATU_PROCON  ORDER BY TELEFONE
	...
	update ATU_PROCON set obs = 'ddd invalido' where obs is null and  ddd not in ( select ddd from UFBR where uf = 'RN' group by ddd )
	...
	insert into DoNotCall_RN ( ddd ,telefone ,telbase ,data ) 
