	...
	use procon
		select ddd from UFBR where uf = 'MS' group by ddd 
	...
	update ATU_PROCON set obs = 'ddd invalido' where obs is null and  ddd not in ( select ddd from UFBR where uf = 'MS' group by ddd )
	...
	insert into DoNotCall_MS ( ddd ,telefone ,telbase ,data ) 
