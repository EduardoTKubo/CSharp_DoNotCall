	...
	use procon
		...
		select ddd from UFBR where uf = 'RS' group by ddd -- 51 53 54 55
		select * from ATU_PROCON  ORDER BY FONE
		select * from ATU_PROCON where len(fone) = 8
		select * from ATU_PROCON where len(fone) = 9
		select * from ATU_PROCON where len(fone) < 10
		select * from ATU_PROCON where len(fone) > 11
	...
	update ATU_PROCON set obs = 'ddd invalido' where obs is null and  ddd not in ( select ddd from UFBR where uf = 'RS' group by ddd )
	...
	insert into DoNotCall_RS ( ddd ,telefone ,telbase ,data ) 
	select ddd ,fone ,ddd + fone ,convert(nvarchar ,data ,23)  from ATU_PROCON order by id

		select top 3 * from ATU_PROCON
		select top 3 * from DoNotCall_RS order by codcall desc -- 230096


		select * into #dupl from ( 
		select * ,row_number() over(partition by telbase order by telbase) as seq from DoNotCall_RS 
							  ) as x where seq > 1
