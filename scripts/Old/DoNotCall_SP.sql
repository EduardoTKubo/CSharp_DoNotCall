	...
	use procon
		...
		select ddd from UFBR where uf = 'SP' group by ddd -- 11 a 19
	...
	update ATU_PROCON set obs = 'ddd invalido' where obs is null and  ddd not in ( select ddd from UFBR where uf = 'SP' group by ddd )
	...
	insert into DoNotCall_SP ( ddd ,telefone ,telbase ,data ) 
	select ddd ,fone ,ddd + fone ,convert(nvarchar ,data ,103)  from ATU_PROCON order by id


		select top 5 * from ATU_PROCON 
		select top 5 * from DoNotCall_SP order by codcall desc 


		select * into #dupl from ( 
		select * ,row_number() over(partition by telbase order by telbase) as seq from DoNotCall_SP
							  ) as x where seq > 1

		select * from #dupl order by telbase 
		delete from DoNotCall_SP where codCALL in ( select codcall from #dupl );
		drop table #dupl;

		select count(*) from DoNotCall_SP -- 2.984.087
