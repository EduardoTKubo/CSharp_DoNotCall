	...
	use procon
		...
		select ddd from UFBR where uf = 'PR' group by ddd 
	...
	update ATU_PROCON set obs = 'ddd invalido' where obs is null and  ddd not in ( select ddd from UFBR where uf = 'PR' group by ddd )
	...
	insert into DoNotCall_PR ( ddd ,telefone ,telbase ,data ) 
